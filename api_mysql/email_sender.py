...
def get_ses_client():
    return boto3.client(
        "ses",
        region_name=str(AWS_REGION) if AWS_REGION else "us-east-1",
        aws_access_key_id=str(AWS_ACCESS_KEY_ID) if AWS_ACCESS_KEY_ID else None,
        aws_secret_access_key=(
            str(AWS_SECRET_ACCESS_KEY) if AWS_SECRET_ACCESS_KEY else None
        ),
    )
...
def send(recipient, subject, body, sender=None):
    ...
    try:
        ...
        if error_code == "AccessDenied" or "not authorized" in error_message.lower():
            friendly_msg = (
                f"Usuário IAM não autorizado a enviar emails. "
                f"Verifique se a policy 'ses:SendEmail' está anexada ao usuário. "
                f"Erro original: {error_message}"
            )
        elif error_code == "MessageRejected":
            if "not verified" in error_message.lower():
                friendly_msg = (
                    f"Email ou domínio não verificado no SES. "
                    f"Verifique a identidade '{sender}' no console AWS SES. "
                    f"Erro original: {error_message}"
                )
            else:
                friendly_msg = f"Email rejeitado pelo SES: {error_message}"
        elif error_code == "Throttling":
            friendly_msg = "Limite de envio do SES excedido. Aguarde alguns segundos e tente novamente."
        elif error_code == "InvalidParameterValue":
            friendly_msg = f"Parâmetro inválido: {error_message}"
        else:
            friendly_msg = f"Erro ao enviar email: {error_message}"
